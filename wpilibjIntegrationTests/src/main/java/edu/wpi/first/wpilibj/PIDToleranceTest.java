...
public class PIDToleranceTest extends AbstractComsSetup {
  ...
  public void testAbsoluteTolerance() {
    ...
    pid.enable();
    Timer.delay(1);
    assertFalse("Error was in tolerance when it should not have been. Error was " + pid
        .getAvgError(), pid.onTarget());
    inp.val = setPoint + tolerance / 2;
    Timer.delay(1.0);
    assertTrue("Error was not in tolerance when it should have been. Error was " + pid
        .getAvgError(), pid.onTarget());
