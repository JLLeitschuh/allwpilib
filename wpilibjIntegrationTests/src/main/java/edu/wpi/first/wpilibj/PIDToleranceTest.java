...
public class PIDToleranceTest extends AbstractComsSetup {
  ...
  public void testAbsoluteTolerance() {
    ...
    m_pid.enable();
    Timer.delay(1);
    assertFalse("Error was in tolerance when it should not have been. Error was " + m_pid
        .getAvgError(), m_pid.onTarget());
    m_inp.m_val = m_setPoint + m_tolerance / 2;
    Timer.delay(1.0);
    assertTrue("Error was not in tolerance when it should have been. Error was " + m_pid
        .getAvgError(), m_pid.onTarget());
