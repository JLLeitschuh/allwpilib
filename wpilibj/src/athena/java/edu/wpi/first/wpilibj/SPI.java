...
public class SPI extends SensorBase {
  ...
  public SPI(Port port) {
    ...
    SPIJNI.spiInitialize(this.port);
  ...
  public final void setMSBFirst() {
    this.bitOrder = 1;
    SPIJNI.spiSetOpts(port, this.bitOrder, this.dataOnTrailing, this.clockPolarity);
  }
  ...
  public int write(byte[] dataToSend, int size) {
    ...
    return SPIJNI.spiWrite(port, dataToSendBuffer, (byte) size);
  }
  ...
  public int write(ByteBuffer dataToSend, int size) {
    ...
    return SPIJNI.spiWrite(port, dataToSend, (byte) size);
  }
  ...
  public int read(boolean initiate, byte[] dataReceived, int size) {
    ...
    ByteBuffer dataToSendBuffer = ByteBuffer.allocateDirect(size);
    if (initiate) {
      retVal = SPIJNI.spiTransaction(port, dataToSendBuffer, dataReceivedBuffer, (byte) size);
    } else {
      retVal = SPIJNI.spiRead(port, dataReceivedBuffer, (byte) size);
    }
  ...
  public int read(boolean initiate, ByteBuffer dataReceived, int size) {
    ...
    if (initiate) {
      ByteBuffer dataToSendBuffer = ByteBuffer.allocateDirect(size);
      return SPIJNI.spiTransaction(port, dataToSendBuffer, dataReceived, (byte) size);
    }
    return SPIJNI.spiRead(port, dataReceived, (byte) size);
  }
  ...
  public int transaction(byte[] dataToSend, byte[] dataReceived, int size) {
    ...
    dataToSendBuffer.put(dataToSend);
    ByteBuffer dataReceivedBuffer = ByteBuffer.allocateDirect(size);
    int retVal = SPIJNI.spiTransaction(port, dataToSendBuffer, dataReceivedBuffer, (byte) size);
    dataReceivedBuffer.get(dataReceived);
    return retVal;
  }
  ...
  public int transaction(ByteBuffer dataToSend, ByteBuffer dataReceived, int size) {
    ...
    return SPIJNI.spiTransaction(port, dataToSend, dataReceived, (byte) size);
  }
  ...
  public void initAccumulator(double period, int cmd, int xferSize,
    ...
    SPIJNI.spiInitAccumulator(port, (int) (period * 1.0e6), cmd,
  ...
  public void freeAccumulator() {
    SPIJNI.spiFreeAccumulator(port);
  }
  ...
  public void resetAccumulator() {
    SPIJNI.spiResetAccumulator(port);
  }
  ...
  public void setAccumulatorCenter(int center) {
    SPIJNI.spiSetAccumulatorCenter(port, center);
  }
  ...
  public void setAccumulatorDeadband(int deadband) {
    SPIJNI.spiSetAccumulatorDeadband(port, deadband);
  }
  ...
  public int getAccumulatorLastValue() {
    return SPIJNI.spiGetAccumulatorLastValue(port);
  }
  ...
  public long getAccumulatorValue() {
    return SPIJNI.spiGetAccumulatorValue(port);
  }
  ...
  public int getAccumulatorCount() {
    return SPIJNI.spiGetAccumulatorCount(port);
  }
  ...
  public double getAccumulatorAverage() {
    return SPIJNI.spiGetAccumulatorAverage(port);
  }
  ...
  public void getAccumulatorOutput(AccumulatorResult result) {
    ...
    SPIJNI.spiGetAccumulatorOutput(port, value.asLongBuffer(), count.asIntBuffer());
