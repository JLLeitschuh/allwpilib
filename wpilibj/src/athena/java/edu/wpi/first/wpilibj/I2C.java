...
public class I2C extends SensorBase {
   ...
   * @param sendSize     Number of bytes to send as part of the transaction.
   * @param dataReceived Buffer to read data into. Must be allocated using ByteBuffer
   *                     .allocateDirect().
   * @param receiveSize  Number of bytes to read from the device.
   * @return Transfer Aborted... false for success, true for aborted.
   */
   ...
   *
   * This allows you to figure out if there is a device on the I2C bus that responds to the address
  ...
  public synchronized boolean writeBulk(ByteBuffer data, int size) {
    ...
    return I2CJNI
        .i2CWrite((byte) m_port.getValue(), (byte) m_deviceAddress, data,
            (byte) size) < size;
  }
   ...
   *
   * Read bytes from a device. Most I2C devices will auto-increment the register pointer internally
   * allowing you to read consecutive registers on a device in a single transaction.
   *
