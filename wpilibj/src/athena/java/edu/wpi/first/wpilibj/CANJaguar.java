...
public class CANJaguar implements MotorSafety, PIDOutput, CANSpeedController {
   ...
   *
   * <p>The scale and the units depend on the mode the Jaguar is in.<br> In percentVbus mode, the
   ...
   *
   * <p>Outside of position and velocity modes the return value of getError() has relatively little
   * meaning.
   *
   ...
   *
   * <p>Choose encoder as the source of speed feedback when in speed control mode.
   *
   ...
   * Enable the closed loop controller.
   *
   * <p>Start actually controlling the output based on the feedback. This is the same as calling
   ...
   *
   * <p>Change the control mode of this Jaguar object.
   *
   ...
   *
   * <p>Ask the Jagaur what mode it is in.
   *
   ...
   *
   * <p>When in PercentVbus or Voltage output mode, the rate at which the voltage changes can be
   ...
   *
   * <p>Use {@link #configSoftPositionLimits(double, double)} or {@link
   ...
   *
   * <p>Use {@link #configSoftPositionLimits(double, double)} to set this and the {@link LimitMode}
   * automatically.
   *
   * @param forwardLimitPosition The position that, if exceeded, will disable the forward
   *                             direction.
  ...
  final double m_voltageRampRate = 0.0;
   ...
   *
   * <p>Jaguar always generates a message with the same message ID when replying.
   *
  ...
  @SuppressWarnings({"ParameterName", "MethodName", "SummaryJavadoc"})
  public static boolean FXP8_EQ(double a, double b) {
    return (int) (a * 256.0) == (int) (b * 256.0);
  }

  /* Compare floats for equality as fixed point numbers */
  @SuppressWarnings({"ParameterName", "MethodName", "SummaryJavadoc"})
  public static boolean FXP16_EQ(double a, double b) {
