...
public class RobotDrive implements MotorSafety {
  ...
  protected SpeedController rearLeftMotor;
  protected SpeedController rearRightMotor;
  protected boolean allocatedSpeedControllers;
  protected byte syncGroup = 0;
  ...
  public RobotDrive(final int frontLeftMotor, final int rearLeftMotor, final int frontRightMotor,
    ...
    sensitivity = kDefaultSensitivity;
    maxOutput = kDefaultMaxOutput;
    this.rearLeftMotor = new Talon(rearLeftMotor);
    this.rearRightMotor = new Talon(rearRightMotor);
    this.frontLeftMotor = new Talon(frontLeftMotor);
    this.frontRightMotor = new Talon(frontRightMotor);
    allocatedSpeedControllers = true;
    setupMotorSafety();
    drive(0, 0);
  }
  ...
  public RobotDrive(SpeedController leftMotor, SpeedController rightMotor) {
    if (leftMotor == null || rightMotor == null) {
      rearLeftMotor = rearRightMotor = null;
  ...
  public RobotDrive(SpeedController frontLeftMotor, SpeedController rearLeftMotor,
    ...
    if (frontLeftMotor == null || rearLeftMotor == null || frontRightMotor == null
        || rearRightMotor == null) {
      frontLeftMotor = rearLeftMotor = frontRightMotor = rearRightMotor = null;
      throw new NullPointerException("Null motor provided");
    }
  ...
  public void mecanumDrive_Cartesian(double x, double y, double rotation, double gyroAngle) {
    ...
    frontLeftMotor.set(wheelSpeeds[MotorType.kFrontLeft_val] * maxOutput, syncGroup);
    frontRightMotor.set(wheelSpeeds[MotorType.kFrontRight_val] * maxOutput, syncGroup);
    rearLeftMotor.set(wheelSpeeds[MotorType.kRearLeft_val] * maxOutput, syncGroup);
    rearRightMotor.set(wheelSpeeds[MotorType.kRearRight_val] * maxOutput, syncGroup);
  ...
  public void free() {
    if (allocatedSpeedControllers) {
      if (frontLeftMotor != null) {
        ((PWM) frontLeftMotor).free();
      }
      if (frontRightMotor != null) {
        ((PWM) frontRightMotor).free();
      }
      if (rearLeftMotor != null) {
        ((PWM) rearLeftMotor).free();
      }
      if (rearRightMotor != null) {
        ((PWM) rearRightMotor).free();
      }
    }
  }
