...
public abstract class SolenoidBase extends SensorBase {
  ...
  protected final int moduleNumber; // /< The number of the solenoid module being
  // used.
  ...
  public void free() {
    for (int i = 0; i < ports.length; i++) {
      SolenoidJNI.freeSolenoidPort(ports[i]);
      ports[i] = 0;
    }
  }
  ...
  protected synchronized void set(int value, int mask) {
    for (int i = 0; i < SensorBase.kSolenoidChannels; i++) {
      int localMask = 1 << i;
      if ((mask & localMask) != 0) {
        SolenoidJNI.setSolenoid(ports[i], (value & localMask) != 0);
      }
    }
  }
   ...
   */
  public byte getAll() {
    return SolenoidJNI.getAllSolenoids(ports[0]);
  }
  ...
  public byte getPCMSolenoidBlackList() {
    return (byte) SolenoidJNI.getPCMSolenoidBlackList(ports[0]);
  }
  ...
  public boolean getPCMSolenoidVoltageStickyFault() {
    return SolenoidJNI.getPCMSolenoidVoltageStickyFault(ports[0]);
  }
  ...
  public boolean getPCMSolenoidVoltageFault() {
    return SolenoidJNI.getPCMSolenoidVoltageFault(ports[0]);
  }
  ...
  public void clearAllPCMStickyFaults() {
    SolenoidJNI.clearAllPCMStickyFaults(ports[0]);
  }
}
