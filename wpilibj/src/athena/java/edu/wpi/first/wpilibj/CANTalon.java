...
public class CANTalon implements MotorSafety, PIDOutput, PIDSource, CANSpeedController {
   ...
   * is also cleared.
   *
   * Typical workflow: set(Disable), Confirm Disable takes effect, clear buffer and push buffer
  ...
  public static class TrajectoryPoint {
     ...
     *
     * Additionaly setting this basically gives you direct control of the motor output since motor
     * output = targetVelocity X Kv, where Kv is our Fgain. This means you can also scheduling
     ...
     *
     * Otherwise you can leave this false for all points, and offset the positions of all trajectory
     * points so they are correct.
     */
  ...
  public static class MotionProfileStatus {
     ...
     *
     * The robot API holds a "top buffer" of trajectory points, so your applicaion can dump several
     * points at once.  The API will then stream them into the Talon's low-level buffer, allowing
     * the Talon to act on them.
     */
   ...
   *
   * In PercentVbus, the output is between -1.0 and 1.0, with 0.0 as stopped. In Follower mode, the
   * output is the integer device ID of the talon to duplicate. In Voltage mode, outputValue is in
   * volts. In Current mode, outputValue is in amperes. In Speed mode, outputValue is in position
      ...
      switch (m_controlMode) {
        ...
        case Speed:
          CanTalonJNI.SetDemand(m_handle, ScaleVelocityToNativeUnits(m_feedbackDevice,
              (isInverted ? -outputValue : outputValue)));
          break;
        case Position:
   ...
   *
   * The only difference between this and {@link PIDController#reset} is that the PIDController also
   * resets the previous error for the D term, but the difference should have minimal effect as it
   ...
   *
   * This only affects position and velocity closed loop control. Allows for situations where you
   ...
   *
   * In Current mode: returns output current. In Speed mode: returns current speed. In Position
   ...
   *
   * @return The 24bit analog position. The bottom ten bits is the ADC (0 - 1023) on the analog pin
   * of the Talon. The upper 14 bits tracks the overflows and underflows (continuous sensor).
   */
   ...
   *
   * @return The position of the sensor currently providing feedback. When using analog sensors, 0
   * units corresponds to 0V, 1023 units corresponds to 3.3V When using an analog encoder (wrapping
   * around 1023 to 0 is possible) the units are still 3.3V per 1023 units. When using quadrature,
   ...
   *
   * Limits the rate at which the throttle will change. Only affects position and speed closed loop
   * modes.
   *
   ...
   *
   * Whenever the error is larger than the izone value, the accumulated integration error is cleared
   * so that high errors aren't racked up when at high errors. An izone value of 0 means no
   ...
   *
   * Since Talon saves setting to flash this should only affect a given Talon initially during robot
   * install.
   *
  ...
  /**
   * @return Number of native units per rotation if scaling info is available. Zero if scaling
  ...
  double GetNativeUnitsPerRotationScalar(FeedbackDevice devToLookup) {
    ...
    switch (devToLookup) {
      case QuadEncoder: { /* When caller wants to lookup Quadrature, the QEI may be in 1x if the
      selected feedback is edge counter.
   ...
   *                          Otherwise MP executer will eventuall see an empty buffer after the
   *                          last point expires, causing it to assert the IsUnderRun flag. However
   *                          this may be desired if calling application nevers wants to terminate
   *                          the MP.
   * @param zeroPos           set to nonzero to signal Talon to "zero" the selected position sensor
   *                          before executing this trajectory point.  Typically the first point
   *                          should have this set only thus allowing the remainder of the MP
   *                          positions to be relative to zero.
   ...
   *              Caller should must instantiate the motionProfileStatus object first then pass into
   *              this routine to be filled.
   */
   ...
   */
  protected void setMotionProfileStatusFromJNI(MotionProfileStatus motionProfileStatus, int
      flags, int profileSlotSelect, int targPos, int targVel, int topBufferRem, int topBufferCnt,
    ...
    motionProfileStatus.activePointValid = ((flags & CanTalonJNI
        .kMotionProfileFlag_ActTraj_IsValid) > 0);
    motionProfileStatus.activePoint.isLastPoint = ((flags & CanTalonJNI
        .kMotionProfileFlag_ActTraj_IsLast) > 0);
    motionProfileStatus.activePoint.velocityOnly = ((flags & CanTalonJNI
        .kMotionProfileFlag_ActTraj_VelOnly) > 0);
    motionProfileStatus.activePoint.position = ScaleNativeUnitsToRotations(m_feedbackDevice,
        targPos);
