 ...
 *
 * Filters are of the form: y[n] = (b0*x[n] + b1*x[n-1] + ... + bP*x[n-P) - (a0*y[n-1] + a2*y[n-2] +
 * ... + aQ*y[n-Q])
 *
 * Where: y[n] is the output at time "n" x[n] is the input at time "n" y[n-1] is the output from the
 * LAST time step ("n-1") x[n-1] is the input from the LAST time step ("n-1") b0...bP are the
 ...
 *
 * What can linear filters do? Basically, they can filter, or diminish, the effects of undesirable
 * input frequencies. High frequencies, or rapid changes, can be indicative of sensor noise or be
 * otherwise undesirable. A "low pass" filter smooths out the signal, reducing the impact of these
 * high frequency components.  Likewise, a "high pass" filter gets rid of slow-moving signal
 ...
 * .org/wiki/Linear_filter
 * http://en.wikipedia.org/wiki/Iir_filter http://en.wikipedia.org/wiki/Fir_filter
 *
 ...
 * periodic function.
 *
 * Note 2: For ALL filters, gains are necessarily a function of frequency. If you make a filter that
 * works well for you at, say, 100Hz, you will most definitely need to adjust the gains if you then
 * want to run it at 200Hz! Combining this with Note 1 - the impetus is on YOU as a developer to
 * make sure PIDGet() gets called at the desired, constant frequency!
 */
