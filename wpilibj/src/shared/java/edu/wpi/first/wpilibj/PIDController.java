...
public class PIDController implements PIDInterface, LiveWindowSendable, Controller {
  ...
  public class PercentageTolerance implements Tolerance {
    ...
    @Override
    public boolean onTarget() {
      return isAvgErrorValid() && (Math.abs(getAvgError()) < percentage / 100 * (m_maximumInput -
          m_minimumInput));
    }
  }
   ...
   *
   * Both of the provided feed forward calculations are velocity feed forwards. If a different feed
   ...
   * @deprecated Use {@link #setPercentTolerance(double)} or {@link #setAbsoluteTolerance(double)}
   * instead.
   */
  @Deprecated
